...
for(const p of perms){
 ...
 let buf;
 try { buf=Buffer.from(joined,'base64'); } catch { continue; }
 console.log('CLASSICAL-DIAG decoded',p.join(''),'bytes',buf.length,'magic',buf.subarray(0,8).toString('hex'));
 for(const [kind,fn] of [['gunzip',gunzipSync],['inflate',inflateSync],['brotli',brotliDecompressSync]]){
  try{
   const out=fn(buf);
   console.log('CLASSICAL-DIAG SUCCESS',p.join(''),kind,'bytes',out.length,'head',out.subarray(0,180).toString('utf8').replace(/\s+/g,' '));
   try{
...
if(!found) console.log('CLASSICAL-DIAG no complete decoded compressed stream found in the three stored chunks.');
