...
assert.equal(report.translatedPdfUniqueFileCount,14,'Fourteen exact translated/reception PDFs after Maulail promotion.');
...
for(const [id,pdf,author] of corrected)for(const prefix of ['','en/']){
  const file=path.join(ROOT,prefix,'bibliography/resources',id,'index.html');assert(fs.existsSync(file),`Corrected resource page: ${file}`);const html=fs.readFileSync(file,'utf8');assert(html.includes(pdf),`${pdf} visible`);assert(html.includes(author),`${author} visible`);assert(html.includes('Gajendra Thakur'),`Gajendra Thakur translator visible: ${file}`);assert(html.includes('id="scholarly-relation"'),`Scholarly relation section: ${file}`);assert(html.includes('application/ld+json'),`JSON-LD: ${file}`);
}
...
console.log('Editor attribution validation PASS: Ashish Anchinhar author/editor roles, Jagdish Prasad Mandal Maulail authorship, and multi-author GT/PT criticism are all preserved.');
