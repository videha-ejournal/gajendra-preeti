...
assert.equal(gajendra.items.length,45,'Gajendra Maithili translation corpus must contain exactly 45 entries.');
assert.deepEqual(gajendra.items.map(x=>x.number),Array.from({length:45},(_,i)=>i+1),'Gajendra translation numbering must stay 1..45.');
...
assert.equal(gajendra.items.filter(x=>x.route==='english-direct').length,42,'Exactly items 4–45 must use direct English → Maithili route.');
...
assert.equal(new Set(gajendra.items.map(x=>x.url)).size,45,'All 45 Gajendra translation URLs must remain unique.');
assert(gajendra.items.every(x=>x.url.startsWith('https://')),'All Gajendra translation URLs must remain HTTPS.');
assert(!gajendra.items.some(x=>x.titleMai==='मू परियोजना — देवनागरी लिपि'),'Item 46 must remain outside the 45-entry Maithili translation corpus unless separately confirmed.');
...
for(const [lang,html] of [['mai',mai],['en',en]]){
 ...
 assert(html.includes('data-provenance="gajendra-maithili-translations-45"'),`${lang}: Gajendra 45-entry provenance callout missing.`);
...
assert(en.includes('Items 4–45: English → Maithili by Gajendra Thakur'));
assert(mai.includes('१–३: विभिन्न भारतीय भाषाक मूलसँ English माध्यमेँ मैथिली'));
assert(mai.includes('४–४५: English सँ सीधे मैथिली'));
...
assert(structured.includes("#gajendra-maithili-translations-45"),'Gajendra 45-entry translation corpus must be present in JSON-LD.');

console.log('Author corpus provenance PASS: Preeti 4 original Maithili + 12 English→Maithili translations; Gajendra 45 Maithili translations with items 1–3 via English and 4–45 direct from English.');
