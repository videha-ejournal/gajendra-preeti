...
const sanskritFiles=['sanskrit-classics-01-05.json','sanskrit-classics-06-09.json','sanskrit-classics-10-13.json'];
const sanskrit=sanskritFiles.flatMap(file=>readJson(file).records||[]).sort((a,b)=>a.order-b.order);
...
assert.deepEqual(sanskrit.map(r=>r.id),auditSanskrit.works.map(w=>w.id),'Sanskrit records must match source-audit work sequence.');
assert.deepEqual(rang.records.map(r=>r.id),auditRang.works.map(w=>w.id),'Rang-Sangam records must match source-audit work sequence.');
...
const inspect=(r,label)=>{
  ...
  assert(Array.isArray(r.units)&&r.units.length>0,`${label}/${r.id}: missing source-defined units`);
  for(const u of r.units){assert(String(u.mai||'').trim(),`${r.id}: unit missing Maithili title`);assert(String(u.en||'').trim(),`${r.id}: unit missing English title`);assert(String(u.gistMai||'').trim(),`${r.id}: unit missing Maithili gist`);assert(String(u.gistEn||'').trim(),`${r.id}: unit missing English gist`);}
};
sanskrit.forEach(r=>inspect(r,'sanskrit-classics'));
rang.records.forEach(r=>inspect(r,'rang-sangam'));

const records=[
  ...sanskrit.map(r=>({...r,collectionId:'sanskrit-classics',collectionMai:'संस्कृत साहित्य-संग्रह — १३ पुस्तक',collectionEn:'Sanskrit Literature Collection — 13 Books'})),
  ...rang.records.map(r=>({...r,collectionId:'rang-sangam',collectionMai:rang.collection.titleMai,collectionEn:rang.collection.titleEn}))
];
assert.equal(records.length,22,'Drama & Sanskrit Classics reader must contain exactly 22 works.');
