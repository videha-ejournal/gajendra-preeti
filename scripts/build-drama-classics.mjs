...
const normalizedUnits=r=>{
  ...
  if(Array.isArray(r.subworks)&&r.subworks.length) return r.subworks.map(s=>({
    n:s.n,mai:s.mai,en:s.en,gistMai:s.gistMai,gistEn:s.gistEn,
    unitCount:s.unitCount||null,subunits:Array.isArray(s.units)?s.units:[]
  }));
  return [];
};
const inspect=(r,label)=>{
  ...
  assert(units.length>0,`${label}/${r.id}: missing source-defined units or subworks`);
  for(const u of units){assert(String(u.mai||'').trim(),`${r.id}: unit missing Maithili title`);assert(String(u.en||'').trim(),`${r.id}: unit missing English title`);assert(String(u.gistMai||'').trim(),`${r.id}: unit missing Maithili gist`);assert(String(u.gistEn||'').trim(),`${r.id}: unit missing English gist`);}
};
...
console.log('Built Drama & Sanskrit Classics bilingual reader: 22 works (13 Sanskrit Classics + 9 Rang-Sangam), including composite source subworks.');
