...
for(const rule of manifest.identityOverrides){
 ...
 for(const alias of [rule.canonicalLabel,...rule.sameWorkAliases]){
  const key=normalize(alias), prior=aliasToIsbn.get(key);
  if(prior&&prior!==rule.canonicalIsbn)failures.push(`Alias resolves to two ISBNs: ${alias}`);
  aliasToIsbn.set(key,rule.canonicalIsbn);
 }
}
...
const rowsHtml=expanded.map((r,i)=>`<tr data-q="${esc(normalize([r.isbn,r.title,r.authorEditor,r.language,r.year,r.administrator,aliases.get(r.isbn)||''].join(' ')))}"><td>${i+1}</td><td><code>${esc(r.isbn)}</code>${aliases.has(r.isbn)?`<small class="alias">${esc(aliases.get(r.isbn))}</small>`:''}</td><td>${esc(r.title)}</td><td>${esc(r.authorEditor)}</td><td>${esc(r.language)}</td><td>${esc(r.edition||'—')}</td><td>${esc(r.year)}</td><td>${esc(r.publicationDate||'—')}</td><td>${esc(r.administrator)}</td></tr>`).join('');
