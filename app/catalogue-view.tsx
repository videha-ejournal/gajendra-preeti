...
export default function CatalogueView({english=false}:{english?:boolean}){
 ...
 <div className="results-line"><span role="status" aria-live="polite">{filtered.length} / {works.length} {english?'works shown':'रचना देखाओल'}</span><span>{english?'All works visible by default':'सभ रचना आरम्भेसँ उपलब्ध'}</span></div><div className="work-grid">{filtered.map(w=>{const review=reviews[w.id];const extras=(w as typeof w&{alternates?:{label:string;url:string}[]}).alternates||[];const role=preetiRole(w,english);return <article id={'work-'+w.id} data-citation-title={english?w.titleEn:w.title} className={'work-card '+w.author} key={w.id}><div className="work-top"><span>{english?w.kindEn:w.kind}</span><span>{w.year||'—'}</span></div><div><p className="work-author">{english?w.nameEn:w.name}</p><h3><a href={w.url}>{english?w.titleEn:w.title}<span className="work-arrow" aria-hidden="true">↗</span></a></h3><p className="destination-note" lang="en">{destination(w.url)} ↗</p><p className="edition-meta">{editionStatus(w,english)}</p>{role&&<p className="translation-meta">{role}</p>}</div><div className="work-bottom"><p>{descriptiveNote(w,english)}</p><a href={w.source}>{english?'Source':'स्रोत'} ↗</a></div><div className="work-reference-links"><a href={'#work-'+w.id} aria-label={(english?'Cite anchor for ':'उद्धरण-कड़ी: ')+(english?w.titleEn:w.title)}>{english?'Cite':'उद्धरण'} · #{w.id.toUpperCase()}</a>{review&&<a href={english?'/gajendra-preeti/criticism/en/'+review+'.html':'/gajendra-preeti/criticism/'+review+'.html'}>{english?'English criticism':'मैथिली समालोचना'} →</a>}{extras.map(a=><a href={a.url} key={a.url}>{a.label} ↗</a>)}</div></article>;})}</div>
