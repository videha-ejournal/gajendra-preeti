...
export default function StructuredData({english=false}:{english?:boolean}){
 ...
 const records=works.map(w=>{
  ...
  return {'@type':isBook?'Book':'CreativeWork','@id':base+'#work-'+w.id,name:w.title,alternateName:w.titleEn,url:base+(english?'en/':'')+'#work-'+w.id,genre:w.kindEn,...credit,...(w.year?{datePublished:String(w.year)}:{}),...(w.kindEn==='Translation'?{inLanguage:'mai'}:{}),encoding:{'@type':'MediaObject',contentUrl:w.url},isBasedOn:{'@type':'WebPage',url:w.source}};
 });
