...
export default function StructuredData({english=false}:{english?:boolean}){
 ...
 const records=works.map(w=>{
  ...
  return {'@type':'CreativeWork','@id':base+'#work-'+w.id,name:w.title,alternateName:w.titleEn,url:base+(english?'en/':'')+'#work-'+w.id,genre:w.kindEn,...credit,...(w.year?{datePublished:String(w.year)}:{}),encoding:{'@type':'MediaObject',contentUrl:w.url},isBasedOn:{'@type':'WebPage',url:w.source}};
 });
 const graph={'@context':'https://schema.org','@graph':[...people,...records,{'@type':'CollectionPage','@id':base+(english?'en/':''),url:base+(english?'en/':''),name:english?'Preeti Thakur & Gajendra Thakur — Videha Literary Atlas':'प्रीति ठाकुर आ गजेन्द्र ठाकुर',inLanguage:english?'en':'mai',about:people.map(p=>({'@id':p['@id']})),mainEntity:{'@type':'ItemList',numberOfItems:records.length,itemListElement:records.map((r,i)=>({'@type':'ListItem',position:i+1,item:{'@id':r['@id']}}))}}]};
